...
function updateSponsorPositionAndPerp(perpAddress: Address, sponsorAddress: Address): void {
  updatePerp(perpAddress);
  let positionId = sponsorAddress.toHexString().concat("-").concat(perpAddress.toHexString());
...
function updatePerp(perpAddress: Address): void {
  ...
  perp.totalPositionCollateral = collateral.reverted
    ...
    : toDecimal(collateral.value.rawValue, collateralToken.decimals);
  perp.cumulativeFeeMultiplier = feeMultiplier.reverted ? perp.cumulativeFeeMultiplier : toDecimal(feeMultiplier.value);
